...
class DatasetMR(Dataset):
    ...
    """
    def __init__(self, image_root, dset_name, data_path, v_feat_dirs, q_feat_dir, v_feat_dim, q_feat_dim,
                 q_feat_type="last_hidden_state",
                 max_q_l=32, max_v_l=75, data_ratio=1.0, ctx_mode="video",
                 normalize_v=True, normalize_t=True, load_labels=True,
                 clip_len=2, max_windows=5, span_loss_type="l1", txt_drop_ratio=0,
        ...
        if self.use_cache > 0:
            print('Loading the off-line features...')
            dset_dir = os.path.join('/mnt/bn/duanbaishan-data/gengwenjia/dataset', self.dset_name)
            vid_keys = [meta['vid'] for meta in self.data]
            qid_keys = [meta['qid'] for meta in self.data]
    ...
    def __getitem__(self, index):
        ...
        if self.use_video:
            ctx_l = int(meta['duration']/self.clip_len)
        else:
            ctx_l = self.max_v_l

        '''加入异常处理逻辑'''
        if  ctx_l != 75:
            print('filter problem video')
        ...
        model_inputs["query"] = meta["query"]
        '''下面给inputs原始图片的输入'''
        video_id = meta["vid"]
        parent_path = self.image_root
        all_image = []
        for i in range(0, ctx_l*self.clip_len, self.clip_len):
            current_path = os.path.join(parent_path, video_id, str(i) + '.jpg')
            try:
                ...
                return self.__getitem__(index + 1)
            '''进行图片转换'''
            image = F.to_tensor(current_image)
            resized_image = F.resize(image, (224, 224))
            all_image.append(resized_image)
        
        model_inputs["image"] = torch.stack(all_image, dim = 0)
        # '''进行padding'''
        # stacked_images = model_inputs["image"]
        
        # current_batch_size = stacked_images.shape[0]

        # # 如果当前批次大小小于目标批次大小，需要进行填充
        # if current_batch_size < 75:
        #     # 计算需要填充的图像数量
        #     num_padding_images = 75- current_batch_size
            
        #     # 创建填充图像张量，填充值为 padding_value
        #     padding_shape = (num_padding_images, *stacked_images.shape[1:])
        #     padding_images = torch.full(padding_shape, 0)
            
        #     # 将原图像张量和填充图像张量进行拼接
        #     padded_images = torch.cat([stacked_images, padding_images], dim=0)
        # else:
        #     # 如果当前批次大小已经达到或超过目标批次大小，则直接返回
        #     padded_images = stacked_images
        # model_inputs["image"] = padded_images
        # if padded_images.shape[0] != 75:
        #     print('padding problem')
        #     return self.__getitem__(index + 1)
        '''测试原始模型的输出，测试结束可以去掉'''
        model_inputs["image_path"] = os.path.join(parent_path, video_id)
...
def start_end_collate_mr(batch):
    ...
    for k in model_inputs_keys:
        ...
        if k in ["saliency_pos_labels", "saliency_neg_labels"]:
            ...
            continue
        if k == "image":
            batched_data[k] = torch.cat([e["model_inputs"][k] for e in batch], dim=0)
            continue
        if  k == "query":
...
def prepare_batch_inputs_mr(batched_model_inputs, device, non_blocking=False):
    ...
    '''这里后续需要改进'''
    model_inputs = dict(
        image = batched_model_inputs['image'].cuda(),
        text_input = tuple(batched_model_inputs['query'][i // 75] for i in range(4 * 75)),
        image_path = batched_model_inputs['image_path'],
    )
